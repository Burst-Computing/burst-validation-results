...
if __name__ == "__main__":
    ...
    for df in dfs:
        ...
        lats = df["worker_func_start_tstamp"] - time_zero
        times.append(lats)
        mins.append((lats).min())
        maxs.append((lats).max())
    ...
    for service, lats in zip(SERVICES, times):
        mad = (lats - lats.median()).abs().median()
        iqr = lats.quantile(0.75) - lats.quantile(0.25)
        idr = lats.quantile(0.9) - lats.quantile(0.1)
        ar = lats.max() - lats.min()
        print(f"{service} -  MAD: {mad} IQR: {iqr} IDR: {idr} Range: {ar}")
